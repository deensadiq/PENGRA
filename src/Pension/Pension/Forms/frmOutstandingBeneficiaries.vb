...
Namespace Forms
    ...
    Public Class frmOutstandingBeneficiaries
        Dim I As Integer
        ...
        Private Sub CreateSQLString()
            ...
            If iBenefit = 0 And iYear = 0 And iMonth = 0 Then
                ...
                strSQL = strSQL + " ORDER BY p.AYEAR DESC, p.AMONTH DESC"
                ...
                strSQLTotal = strSQLTotal + " FROM SP_BENEFICIARIES_OUTSTANDING p"

                strEmployee = "SELECT * FROM EMPLOYEEINFO WHERE LIMIT = '1' AND ACTVE = '1' ORDER BY EXTRACT(YEAR FROM DOAA) DESC, EXTRACT(MONTH FROM DOAA) DESC"
            ...
            ElseIf iBenefit <> 0 And iYear = 0 And iMonth = 0 Then
                ...
                strSQL = strSQL + " p.MONTHNAME || ' ' || P.AYEAR AS DESCRIPTION, p.GRATUITY, p.PENSIONARREARS, p.DEATHPENSION, p.TOTALBENEFIT,"
                strSQL = strSQL + " p.GRATUITYPAID, p.PEANSIONPAID, p.DEATHPAID, p.TOTALPAID, p.BALANCE, p.STATUS FROM SP_BENEFICIARIES_OUTSTANDING p "
                strSQL = strSQL + " WHERE p.BTYPE = '" & iBenefit & "' ORDER BY p.AYEAR DESC, p.AMONTH DESC"

                strSQLTotal = "SELECT SUM(p.GRATUITY) AS GRATUITY, SUM(p.PENSIONARREARS) AS PENSIONARREARS, SUM(p.DEATHPENSION) AS DEATHPENSION, SUM(p.TOTALBENEFIT) AS TOTALBENEFIT, "
                strSQLTotal = strSQLTotal + " SUM(p.GRATUITYPAID) AS GRATUITYPAID, SUM(p.PEANSIONPAID) AS PEANSIONPAID, SUM(p.DEATHPAID) AS DEATHPAID, SUM(p.TOTALPAID) AS TOTALPAID, SUM(p.BALANCE) AS BALANCE"
                strSQLTotal = strSQLTotal + " FROM SP_BENEFICIARIES_OUTSTANDING p WHERE p.BTYPE = '" & iBenefit & "'"

                strEmployee = "SELECT * FROM EMPLOYEEINFO WHERE LIMIT = '1' AND ACTVE = '1' AND BENEFITTYPE = '" & iBenefit & "'"
            ...
            ElseIf iBenefit = 0 And iYear <> 0 And iMonth = 0 Then
                ...
                strSQL = strSQL + " p.MONTHNAME || ' ' || P.AYEAR AS DESCRIPTION, p.GRATUITY, p.PENSIONARREARS, p.DEATHPENSION, p.TOTALBENEFIT,"
                strSQL = strSQL + " p.GRATUITYPAID, p.PEANSIONPAID, p.DEATHPAID, p.TOTALPAID, p.BALANCE, p.STATUS FROM SP_BENEFICIARIES_OUTSTANDING p "
                strSQL = strSQL + " WHERE p.AYEAR = '" & iYear & "' ORDER BY p.AYEAR DESC, p.AMONTH DESC"

                strSQLTotal = "SELECT SUM(p.GRATUITY) AS GRATUITY, SUM(p.PENSIONARREARS) AS PENSIONARREARS, SUM(p.DEATHPENSION) AS DEATHPENSION, SUM(p.TOTALBENEFIT) AS TOTALBENEFIT, "
                strSQLTotal = strSQLTotal + " SUM(p.GRATUITYPAID) AS GRATUITYPAID, SUM(p.PEANSIONPAID) AS PEANSIONPAID, SUM(p.DEATHPAID) AS DEATHPAID, SUM(p.TOTALPAID) AS TOTALPAID, SUM(p.BALANCE) AS BALANCE"
                strSQLTotal = strSQLTotal + " FROM SP_BENEFICIARIES_OUTSTANDING p WHERE p.AYEAR = '" & iYear & "'"

                strEmployee = "SELECT * FROM EMPLOYEEINFO WHERE LIMIT = '1' AND ACTVE = '1' AND EXTRACT(YEAR FROM DOAA) = '" & iYear & "'"
            ...
            ElseIf iBenefit <> 0 And iYear <> 0 And iMonth = 0 Then
                ...
                strSQL = strSQL + " p.MONTHNAME || ' ' || P.AYEAR AS DESCRIPTION, p.GRATUITY, p.PENSIONARREARS, p.DEATHPENSION, p.TOTALBENEFIT,"
                strSQL = strSQL + " p.GRATUITYPAID, p.PEANSIONPAID, p.DEATHPAID, p.TOTALPAID, p.BALANCE, p.STATUS FROM SP_BENEFICIARIES_OUTSTANDING p "
                strSQL = strSQL + " WHERE p.BTYPE = '" & iBenefit & "' AND p.AYEAR = '" & iYear & "' ORDER BY p.AYEAR DESC, p.AMONTH DESC"

                strSQLTotal = "SELECT SUM(p.GRATUITY) AS GRATUITY, SUM(p.PENSIONARREARS) AS PENSIONARREARS, SUM(p.DEATHPENSION) AS DEATHPENSION, SUM(p.TOTALBENEFIT) AS TOTALBENEFIT, "
                strSQLTotal = strSQLTotal + " SUM(p.GRATUITYPAID) AS GRATUITYPAID, SUM(p.PEANSIONPAID) AS PEANSIONPAID, SUM(p.DEATHPAID) AS DEATHPAID, SUM(p.TOTALPAID) AS TOTALPAID, SUM(p.BALANCE) AS BALANCE"
                strSQLTotal = strSQLTotal + " FROM SP_BENEFICIARIES_OUTSTANDING p WHERE p.BTYPE = '" & iBenefit & "' AND p.AYEAR = '" & iYear & "'"

                strEmployee = "SELECT * FROM EMPLOYEEINFO WHERE LIMIT = '1' AND ACTVE = '1' AND BENEFITTYPE = '" & iBenefit & "' AND EXTRACT(YEAR FROM DOAA) = '" & iYear & "'"
            ...
            ElseIf iBenefit = 0 And iYear <> 0 And iMonth <> 0 Then
                ...
                strSQL = strSQL + " p.MONTHNAME || ' ' || P.AYEAR AS DESCRIPTION, p.GRATUITY, p.PENSIONARREARS, p.DEATHPENSION, p.TOTALBENEFIT,"
                strSQL = strSQL + " p.GRATUITYPAID, p.PEANSIONPAID, p.DEATHPAID, p.TOTALPAID, p.BALANCE, p.STATUS FROM SP_BENEFICIARIES_OUTSTANDING p "
                strSQL = strSQL + " WHERE p.AYEAR = '" & iYear & "' AND p.AMONTH = '" & iMonth & "' ORDER BY p.AYEAR DESC, p.AMONTH DESC"

                strSQLTotal = "SELECT SUM(p.GRATUITY) AS GRATUITY, SUM(p.PENSIONARREARS) AS PENSIONARREARS, SUM(p.DEATHPENSION) AS DEATHPENSION, SUM(p.TOTALBENEFIT) AS TOTALBENEFIT, "
                strSQLTotal = strSQLTotal + " SUM(p.GRATUITYPAID) AS GRATUITYPAID, SUM(p.PEANSIONPAID) AS PEANSIONPAID, SUM(p.DEATHPAID) AS DEATHPAID, SUM(p.TOTALPAID) AS TOTALPAID, SUM(p.BALANCE) AS BALANCE"
                strSQLTotal = strSQLTotal + " FROM SP_BENEFICIARIES_OUTSTANDING p WHERE p.AYEAR = '" & iYear & "' AND p.AMONTH = '" & iMonth & "'"

                strEmployee = "SELECT * FROM EMPLOYEEINFO WHERE LIMIT = '1' AND ACTVE = '1' AND EXTRACT(YEAR FROM DOAA) = '" & iYear & "' AND EXTRACT(MONTH FROM DOAA) = '" & iMonth & "'"
            ...
            ElseIf iBenefit <> 0 And iYear <> 0 And iMonth <> 0 Then
                ...
                strSQL = strSQL + " p.MONTHNAME || ' ' || P.AYEAR AS DESCRIPTION, p.GRATUITY, p.PENSIONARREARS, p.DEATHPENSION, p.TOTALBENEFIT,"
                strSQL = strSQL + " p.GRATUITYPAID, p.PEANSIONPAID, p.DEATHPAID, p.TOTALPAID, p.BALANCE, p.STATUS FROM SP_BENEFICIARIES_OUTSTANDING p "
                strSQL = strSQL + " WHERE p.BTYPE = '" & iBenefit & "' AND p.AYEAR = '" & iYear & "' AND p.AMONTH = '" & iMonth & "' ORDER BY p.AYEAR DESC, p.AMONTH DESC"

                strSQLTotal = "SELECT SUM(p.GRATUITY) AS GRATUITY, SUM(p.PENSIONARREARS) AS PENSIONARREARS, SUM(p.DEATHPENSION) AS DEATHPENSION, SUM(p.TOTALBENEFIT) AS TOTALBENEFIT, "
                strSQLTotal = strSQLTotal + " SUM(p.GRATUITYPAID) AS GRATUITYPAID, SUM(p.PEANSIONPAID) AS PEANSIONPAID, SUM(p.DEATHPAID) AS DEATHPAID, SUM(p.TOTALPAID) AS TOTALPAID, SUM(p.BALANCE) AS BALANCE"
                strSQLTotal = strSQLTotal + " FROM SP_BENEFICIARIES_OUTSTANDING p WHERE p.BTYPE = '" & iBenefit & "' AND p.AYEAR = '" & iYear & "' AND p.AMONTH = '" & iMonth & "'"

                strEmployee = "SELECT * FROM EMPLOYEEINFO WHERE LIMIT = '1' AND ACTVE = '1' AND BENEFITTYPE = '" & iBenefit & "' AND EXTRACT(YEAR FROM DOAA) = '" & iYear & "' AND EXTRACT(MONTH FROM DOAA) = '" & iMonth & "'"
        ...
        Private Sub dgvGrid_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)

            If e.RowIndex = -1 Then Exit Sub
            If Table.Rows.Count = 0 Then Exit Sub

            'Ignor clicks that are not on button cell
            If e.ColumnIndex < 0 OrElse e.ColumnIndex = dgvGrid.Columns("cEdit").Index Then
            ...
            'Ignor clicks that are not on button cell
            If e.ColumnIndex < 0 OrElse e.ColumnIndex = dgvGrid.Columns("cDetails").Index Then
                Dim rowNo As Integer = e.RowIndex + 1
                ...
                Dim Employee As Integer = Table.Rows(e.RowIndex).Item("EMPLOYEE")
                'Dim form As New frmPaymentStatus
                'form.btnSearch.Visible = False
        ...
        Private Sub dgvGrid_CellPainting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellPaintingEventArgs)
        ...
        Private Sub dgvGrid_Paint(ByVal sender As Object, ByVal e As System.Windows.Forms.PaintEventArgs)
